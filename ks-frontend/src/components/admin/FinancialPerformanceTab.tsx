...
import { format, startOfMonth, startOfQuarter, startOfYear, subDays, differenceInBusinessDays, eachMonthOfInterval, startOfYear as startOfYearFn } from "date-fns";
import { getBusinessDaysInMonth } from "@/utils/utilizationCalculator";
...
export function FinancialPerformanceTab() {
  ...
  const loadFinancialMetrics = async () => {
    try {
      ...
      // Calculate total revenue (billable hours * hourly rates)
      ...
      // Calculate total costs (all hours * cost rates)
      const totalCost = timeEntries?.reduce((sum, entry) => {
        const costRate = entry.profiles?.cost_rate || 0;
        return sum + (entry.hours * costRate);
      }, 0) || 0;

      // Calculate total estimated revenue (sum of estimated hours * rates for all active matters)
      ...
      // Calculate total estimated costs (estimated hours * cost rates)
      ...
      // Calculate total base costs (cost rate * 7.5 * business days for each resource)
      ...
      profiles?.forEach(profile => {
        ...
        // Determine the actual start date for this profile (later of filter start or profile creation)
        const actualStartDate = profileCreatedAt > periodStartDate ? profileCreatedAt : periodStartDate;
        
        // Calculate business days from actual start to now (or end of period)
        const businessDays = differenceInBusinessDays(now, actualStartDate);
        
        // 7.5 hours per day * cost rate * business days
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Financial Overview Cards - Reordered as requested */}
        ...
        <Card className="premium-card">
          <CardContent className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="text-2xl font-bold text-foreground">
                  {loading ? "..." : formatCurrency(metrics.totalCost)}
                </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">Total Base Cost</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {/* Analytics Dashboard - Always Displayed */}
      <Card>
        <CardHeader>
